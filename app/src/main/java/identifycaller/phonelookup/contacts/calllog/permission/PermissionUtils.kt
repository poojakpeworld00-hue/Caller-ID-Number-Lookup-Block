...
object PermissionUtils {
    ...
    val CATALOG: Map<String, PermissionSpec> = listOf(
        ...
        PermissionSpec(
            ...
            // Respect ADHomeActivity's geo gate: READ_PHONE_STATE is only asked
     ...
     * named `AdsPreferance` boolean must be true (defaults to false when unset).
     */
    fun isPrefGateOpen(context: Context, spec: PermissionSpec): Boolean {
        val gate = spec.enabledPrefGate ?: return true
        return AdsPreferance.getInstance(context).getBoolean(gate)
    }
