 ...
 *
 * Started by [CallReceiver] on RINGING and stopped on OFFHOOK/IDLE.
 */
class CallerOverlayService : Service() {
    ...
    private fun showOverlay(number: String) {
        ...
        scope.launch {
            val info = withContext(Dispatchers.IO) { CallerCard.resolve(this@CallerOverlayService, number) }
            overlayView?.let { CallerCard.bind(this@CallerOverlayService, it, number, info) }
        }
    }
