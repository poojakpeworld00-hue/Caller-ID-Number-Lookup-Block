...
/**
 * The MainActivity Full-Screen-Intent priming dialog. Same visual language as the
 * Screen (incoming-call preview + remote-driven copy). "Enable" opens the system
 * FSI page in-task (via MainActivity's launcher) and arms [FsiWatchService]; the
 * dialog dismisses so the auto-return lands on a clean MainActivity.
 *
 * A single live dialog is tracked so the host can [dismissIfShowing] on return.
 */
object FsiPrimingDialog {
    ...
    fun show(
        ...
        view.findViewById<TextView>(R.id.fsDialogButton).setOnClickListener {
            ...
            // only after that launch FSI settings in-task via MainActivity's
            // launcher. The watcher + MainActivity.onResume handle the return.
            enableTapped = true
            dialog.dismiss()
            PermissionEngine.request(activity, "notification") {
                (activity as? MainActivity)?.openFsiSettings()
            }
        }
