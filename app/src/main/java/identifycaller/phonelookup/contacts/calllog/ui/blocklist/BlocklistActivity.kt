...
import identifycaller.phonelookup.contacts.calllog.databinding.ActivityBlocklistBinding
import identifycaller.phonelookup.contacts.calllog.databinding.DialogBlockAddBinding
import identifycaller.phonelookup.contacts.calllog.databinding.DialogBlockDetailsBinding
import identifycaller.phonelookup.contacts.calllog.databinding.DialogBlockMethodsBinding
import identifycaller.phonelookup.contacts.calllog.databinding.DialogBlockRecentsBinding
import identifycaller.phonelookup.contacts.calllog.databinding.DialogEnableCallerIdBinding
import identifycaller.phonelookup.contacts.calllog.databinding.ViewBlockMethodsBinding
...
class BlocklistActivity : BaseActivity<ActivityBlocklistBinding>() {
    ...
    private fun showContactsPicker() {
        val contacts = ContactsRepository(this).getContacts()
            .filter { it.detail.isNotBlank() }
            .map { CallEntry(it.name, it.detail, CallType.OUTGOING, 0L, 0L) }
    ...
    private fun showRecentsPicker() {
        val recents = CallLogRepository(this).getCalls(limit = 200)
    ...
    private fun showPickDialog(
        ...
        val pickAdapter = BlockPickAdapter { entry ->
            dialog.dismiss()
    ...
    private fun requireCallerId(action: () -> Unit) {
        if (CallerIdManager.isCallerIdEnabled(this)) {
            action()
        } else {
    ...
    private fun requestEnableCallerId() {
        val intent = CallerIdManager.buildEnableIntent(this) ?: run {
    ...
    private fun refreshCallerIdGate() {
        if (!CallerIdManager.isCallerIdEnabled(this)) return
