 ...
 *
 * Source resolution mirrors [identifycaller.phonelookup.contacts.calllog.permission.PermissionRepository]:
 ...
 *
 * **Audience split:** when the user is Organic (`AdsPreferance.OnMaketing == false`)
...
data class FsiConfig(
    ...
    companion object {
        private const val TAG = "FsiConfig"
        private const val LOG = "FSI" // shared debug tag with FsiPermission (adb logcat -s FSI)
        ...
        fun load(context: Context): FsiConfig {
            return try {
                val block = rawBlock()
                if (block == null) {
                    SafeSide.log(LOG, "config: no fullscreen_permission block in Remote Config → DISABLED")
                    return DISABLED
                }
                val base = parse(block)
                val isMarketing = AdsPreferance.getInstance(context).getBoolean("OnMaketing")
                ...
                SafeSide.error(TAG, "Failed to parse $BLOCK; feature disabled", e)
                DISABLED
            }
        }
        ...
        private fun rawEngineJson(): String? {
            return try {
                ...
                SafeSide.error(TAG, "Remote Config read failed", e)
                null
            }
        }
        ...
        private fun audienceRoot(obj: JSONObject): JSONObject {
            val isMarketing = AdsPreferance.getOrNull()?.getBoolean("OnMaketing") ?: false
        ...
        private fun parse(o: JSONObject): FsiConfig {
        ...
        private fun FsiConfig.applyOrganic(org: JSONObject): FsiConfig {
