package identifycaller.phonelookup.contacts.calllog.services.onincomming
...
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.my_main_counter.FloatingViewManager
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.my_main_counter.My_Main_Screen
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.my_main_counter.service.MainlJobService.Companion.NOTIFICATION_ID
import identifycaller.phonelookup.contacts.calllog.R
import identifycaller.phonelookup.contacts.calllog.data.BlocklistManager
import identifycaller.phonelookup.contacts.calllog.ui.incall.IncomingCallActivity
 ...
 *    [My_Main_Screen] (overlay/FGS path) or a full-screen notification fallback.
 *
 * Registered in the manifest (fires when the app is dead) and also dynamically by
 * MainlJobService while the app is alive. A 2-second debounce on IDLE dedupes the
...
class CallReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        when (state) {
            TelephonyManager.EXTRA_STATE_RINGING -> {
                ...
                if (!number.isNullOrBlank() && BlocklistManager(context).isBlocked(number)) {
                    wasBlocked = true
                ...
                if (!number.isNullOrBlank() && Settings.canDrawOverlays(context)) {
                    CallerOverlayService.start(context, number)
     ...
     * the role-based [CallScreeningServiceImpl] is the primary blocker and rejects
    ...
    private fun handlePostCall(
        ...
        CoroutineScope(Dispatchers.Main).launch {
            try {
                if (!AdsPreferance.getInstance(context).getBoolean("HD_VBC_Show")) return@launch
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE && canShowOverlay(context)) {
                    ...
                    try {
                        FloatingViewManager(context).showCallbackScreen(phoneNumber, startTime, endTime, type)
                    } catch (e: Exception) {
    ...
    private fun launchCallbackScreen(
        ...
        try {
            val intent = Intent(context, My_Main_Screen::class.java).apply {
    ...
    private fun showFullScreenNotification(
        ...
        if (My_Main_Screen.isActive || IncomingCallActivity.isActive) {
    ...
    companion object {
        ...
        const val ACTION_CALL_ENDED = "identifycaller.phonelookup.contacts.calllog.CALL_ENDED"
