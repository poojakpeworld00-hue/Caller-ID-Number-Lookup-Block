 ...
 *
 * ```
 * someButton.setOnClickListener { PermissionEngine.check(this) }
 * ```
 *
 ...
 *  2. finds rules that target this Activity by simple name ([ActivityMatcher]),
 *  3. drops permissions that are already granted, not applicable on this SDK, or
 *     already shown when `show_once` is set,
 *  4. orders the rest by `priority` ([PermissionQueue]),
...
object PermissionEngine {
    ...
    fun init(context: Context) {
        try {
            ...
            SafeSide.error(TAG, "init failed", e)
        }
    }
    ...
    fun check(activity: Activity, onComplete: (() -> Unit)? = null) {
        try {
            ...
            else ActivityMatcher.rulesFor(name, allRules)
            if (matched.isEmpty()) {
                SafeSide.log(TAG, "No permission rules for $name")
                complete()
                return
            }

            val prefs = PermissionPrefs(activity)
            val pending = matched.filter { rule -> isStillNeeded(activity, rule, prefs) }
            if (pending.isEmpty()) {
                SafeSide.log(TAG, "All configured permissions already satisfied on $name")
                complete()
                return
            }

            SafeSide.log(TAG, "Queueing ${pending.size} permission(s) for $name")
            running = true
            activeRef = WeakReference(activity)
            processNext(PermissionQueue(pending))
        } catch (e: Exception) {
            SafeSide.error(TAG, "check() failed", e)
            abort()
        }
    }
    ...
    fun request(activity: Activity, key: String, onComplete: (() -> Unit)? = null) {
        try {
            ...
            if (spec == null) {
                SafeSide.log(TAG, "request(): unknown permission key '$key' — skipped")
                fireComplete(onComplete); return
            }
            ...
            if (rule != null && !rule.enabled) {
                SafeSide.log(TAG, "request(): '$key' disabled in config — skipped")
                fireComplete(onComplete); return
            }
            val prefs = PermissionPrefs(activity)
            ...
            SafeSide.log(TAG, "request(): asking '$key' on ${activity::class.java.simpleName}")
            ...
            PermissionLauncher.launch(activity, spec.androidPermission) { granted ->
                ...
                SafeSide.log(TAG, "request(): '$key' granted=$granted")
                fireComplete(onComplete)
            }
        } catch (e: Exception) {
            SafeSide.error(TAG, "request() failed", e)
            fireComplete(onComplete)
        }
    }
    ...
    private fun isStillNeeded(
        ...
        if (spec == null) {
            SafeSide.log(TAG, "Unknown permission key '${rule.key}' — skipped")
            return false
        }
        if (!PermissionUtils.isApplicableOnThisSdk(spec)) return false
        if (!PermissionUtils.isPrefGateOpen(activity, spec)) {
            SafeSide.log(TAG, "Pref gate '${spec.enabledPrefGate}' closed — '${rule.key}' skipped")
            return false
        }
    ...
    private fun processNext(queue: PermissionQueue) {
        ...
        scheduler.schedule(rule.delayMs) {
            ...
            SafeSide.log(TAG, "Requesting '${rule.key}' on ${act::class.java.simpleName}")
            PermissionLauncher.launch(act, spec.androidPermission) { granted ->
                ...
                SafeSide.log(TAG, "Result '${rule.key}' granted=$granted")
    ...
    private fun fireComplete(cb: (() -> Unit)?) {
        ...
        try {
            ...
            SafeSide.error(TAG, "onComplete callback threw", e)
        }
    }
}
