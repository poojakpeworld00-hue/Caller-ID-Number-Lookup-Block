 ...
 *
 * Shared by [identifycaller.phonelookup.contacts.calllog.services.onincomming.CallerOverlayService] (floating window, device unlocked) and
...
object CallerCard {
    ...
    fun resolve(context: Context, number: String): Info {
        val name = runCatching { ContactsRepository(context).lookupNameByNumber(number) }.getOrNull()
    ...
    fun bind(context: Context, root: View, number: String, info: Info) {
        ...
        root.findViewById<TextView>(R.id.tvIncallAvatar).text =
            CallFormat.initials(info.name, number)
