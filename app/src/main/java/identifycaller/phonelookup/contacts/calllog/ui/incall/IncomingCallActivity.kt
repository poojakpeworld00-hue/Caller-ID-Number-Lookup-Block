package identifycaller.phonelookup.contacts.calllog.ui.incall
...
import identifycaller.phonelookup.contacts.calllog.services.CallEndWatcher
import identifycaller.phonelookup.contacts.calllog.services.CallerCard
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.SystemDialogHelper
import identifycaller.phonelookup.contacts.calllog.services.onincomming.CallReceiver
 ...
 * keyguard is up; it self-dismisses when [CallReceiver] broadcasts call end.
 */
class IncomingCallActivity : AppCompatActivity() {
    ...
    private val endReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            if (intent?.action == CallReceiver.ACTION_CALL_ENDED) finish()
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            CallerCard.bind(this@IncomingCallActivity, card, number, info)
        }

        val filter = IntentFilter(CallReceiver.ACTION_CALL_ENDED)
    ...
    companion object {
         ...
         * CallReceiver checks it to suppress a duplicate post-call notification (B2).
         */
        @Volatile
        var isActive = false
