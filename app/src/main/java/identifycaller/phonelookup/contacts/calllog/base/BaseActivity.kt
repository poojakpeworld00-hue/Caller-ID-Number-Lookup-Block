package identifycaller.phonelookup.contacts.calllog.base
...
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.ADHomeActivity
 ...
 * ```
 */
abstract class BaseActivity<DB : ViewDataBinding> : ADHomeActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateNativeAdTheme(AdsPreferance.getInstance(this), AppPrefs.selectedTheme(this).ifEmpty { THEME_SYSTEM })
        ...
        // Note: forward/back interstitials are preloaded in ADHomeActivity, so we
    ...
    protected fun goBack() {
        InterADsBack().showBackAds(this) { performBack() }
    }
    ...
    protected open fun showBottomBanner() {
        ...
        ScreenAdConfig.showAd(this::class.java.simpleName, this, container, shimmer)
    ...
    override fun onResume() {
        super.onResume()
        applyLocale()
        InterADsNormal.handleTabReturn(this)
        // NOTE: no LightHouse.syncPermissionsAsync() here — the SDK syncs
        // permissions internally (≥0.6.4), so an explicit call is redundant.
    }
    /** Applies the saved app language ([PrefsManager.languageTag]) via AppCompat. */
    protected fun applyLocale() {
        val savedLang = AppPrefs.selectedLanguage(this)
    ...
    /** Applies the saved night-mode ([PrefsManager.themeMode]) app-wide. */
    ...
    private fun updateNativeAdTheme(adsPref: AdsPreferance, theme: String) {
