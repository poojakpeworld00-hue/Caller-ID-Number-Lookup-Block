package identifycaller.phonelookup.contacts.calllog.permission
...
import identifycaller.phonelookup.contacts.calllog.ui.intro.IntroDisplayConfig
import identifycaller.phonelookup.contacts.calllog.ui.intro.IntroDisplayPolicy
...
/**
 * Bottom-sheet replacement for MainActivity's old sequential first-run
 ...
 *
 * Self-contained: it owns its own result launchers, so MainActivity only has to
 ...
 * [OverlayUtils]. `phone_state` is only listed when `HD_VBC_Show` is on — the
...
class PermissionSheetDialog : BottomSheetDialogFragment() {
    ...
    private data class Row(
        ...
        /** Requested via [PermissionEngine] (RC-driven) instead of directly. */
    ...
    private val overlayLauncher = registerForActivityResult(
        ...
        val granted = context?.let { ctx -> OverlayUtils.isGranted(ctx) } ?: false
    ...
    private fun buildRows(): List<Row> {
        ...
        // Notification + phone state are handled by the PermissionEngine (see
        ...
        if (AdsPreferance.getInstance(ctx).getBoolean("HD_VBC_Show")) {
            list += Row(
    ...
    private fun requestSingle(row: Row) {
        ...
        when {
            ...
            row.engineManaged -> PermissionEngine.check(requireActivity()) {
    ...
    private fun onContinueClicked() {
        // Notification + phone state are managed by the PermissionEngine; once it
        // finishes, request the sheet's own permissions (call log / contacts) and
        // then the overlay step.
        PermissionEngine.check(requireActivity()) {
    ...
    private fun launchOverlay(finishAfter: Boolean) {
        // Reuse MainActivity's overlay flow — it opens the system page with the
        // NO_HISTORY/EXCLUDE_FROM_RECENTS intent, watches the grant via
        // OverlayWatchService, auto-returns the app, and drops the Settings page.
        ...
        // Fallback (not hosted by MainActivity): own launcher, no auto-back, but
        ...
        runCatching {
            overlayLauncher.launch(OverlayUtils.buildOverlayIntent(requireContext().packageName))
        }.onFailure {
            SafeSide.error("PermissionSheet", "Failed to open overlay settings", it)
            if (finishAfter) finishFlow()
        }
    }
    ...
    companion object {
        ...
        fun hasPending(activity: FragmentActivity): Boolean {
            ...
            if (AdsPreferance.getInstance(activity).getBoolean("HD_VBC_Show") &&
            ...
            if (!OverlayUtils.isGranted(activity)) return true
            return false
        }
         ...
         * screen: the engine records it in [PermissionPrefs], the Home quick
         * actions in [PrefsManager]) and `shouldShowRequestPermissionRationale` is
        ...
        fun isPermanentlyDenied(activity: FragmentActivity, key: String, perm: String): Boolean {
            ...
            val askedAnywhere = PermissionPrefs(activity).wasAsked(key) ||
                PrefsManager(activity).hasRequestedPermission(perm)
         ...
         * Combines [hasPending] with the shared [IntroDisplayPolicy] frequency gate,
         ...
         * [IntroDisplayPolicy.markShown] in [show]. This gate is for the
        ...
        fun shouldAutoShow(activity: FragmentActivity): Boolean {
            ...
            return IntroDisplayPolicy.shouldShowPermissionSheet(activity)
        }
        ...
        fun show(activity: FragmentActivity, onFinished: (() -> Unit)? = null) {
            ...
            IntroDisplayPolicy.markShown(activity, IntroDisplayConfig.PERMISSION_SHEET)
