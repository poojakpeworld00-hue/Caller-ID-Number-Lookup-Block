package identifycaller.phonelookup.contacts.calllog.ui.intro
 ...
 * (resolved from `AdsPreferance.OnMaketing`, matching the FSI config) — each
...
enum class PromptFrequency {
    ...
    companion object {
        fun from(raw: String?): PromptFrequency = when (raw?.trim()?.lowercase()) {
            "always" -> ALWAYS
            "once" -> ONCE
...
data class IntroDisplay(
    val enabled: Boolean,
    val frequency: PromptFrequency,
    val interval: Int
)

object IntroDisplayConfig {
    ...
    fun language(context: Context): IntroDisplay =
        load(context, LANGUAGE, defaultEnabled = true, defaultFrequency = PromptFrequency.ONCE)

    /** Terms defaults to OFF (matches the old `hide_terms = true` default). */
    fun terms(context: Context): IntroDisplay =
        load(context, TERMS, defaultEnabled = false, defaultFrequency = PromptFrequency.ONCE)

    /** Onboarding defaults to OFF (matches the old `hide_intro = true` default). */
    fun onboarding(context: Context): IntroDisplay =
        load(context, ONBOARDING, defaultEnabled = false, defaultFrequency = PromptFrequency.ONCE)
     ...
     */
    fun permissionSheet(context: Context): IntroDisplay =
        load(context, PERMISSION_SHEET, defaultEnabled = true, defaultFrequency = PromptFrequency.ALWAYS)
    ...
    private fun load(
        ...
        val ads = AdsPreferance.getInstance(context)
        ...
        if (obj == null) return IntroDisplay(defaultEnabled, defaultFrequency, 0)

        return IntroDisplay(
            enabled = obj.optBoolean("enabled", defaultEnabled),
            frequency = PromptFrequency.from(
