 ...
 * [FsiPermission.shouldShowScreen]; primes the FSI grant then continues to
 * [EXTRA_NEXT] (default [MainActivity]).
 *
 * "Enable Now" opens the system FSI page and arms [FsiWatchService]; when the
 * toggle flips the broadcast reaches [FsiReturnWatcher], which pulls this Activity
 * back — `onResume` then detects the grant and moves on. "Not now" (or back) just
 * continues without the grant.
 */
class FsiPermissionActivity : AppCompatActivity() {

    private val config by lazy { FsiConfig.load(this) }
    private val returnWatcher by lazy { FsiReturnWatcher(this) }
    ...
    // finishAfterSettings).
    //
    // (This replaces relying on FsiWatchService: a background Service can't even be
    ...
    private val grantPoll = object : Runnable {
        override fun run() {
            ...
            if (FsiPermission.isGranted(this@FsiPermissionActivity)) {
                SafeSide.log("FSI", "grant poll: GRANTED → continue to next")
                grantPolling = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (returningFromSettings || FsiPermission.isGranted(this)) {
            SafeSide.log("FSI", "Screen onCreate: returning/granted → continue (no render)")
            continueToNext()
            return
        }
        ...
        FsiPermission.markScreenShown(this)
        ...
        findViewById<TextView>(R.id.fsScreenButton).setOnClickListener {
            ...
            PermissionEngine.request(this, "notification") {
     ...
     * Settings page — either because the user GRANTED it ([FsiPermission.isGranted])
    ...
    private fun continueIfBackFromSettings(where: String) {
        if (navigated) return
        val granted = FsiPermission.isGranted(this)
        if (returningFromSettings || granted) {
            SafeSide.log("FSI", "Screen $where: back from settings, granted=$granted → continue")
    ...
    private fun continueToNext() {
        ...
        SafeSide.log("FSI", "Screen continueToNext → ${nextClass.simpleName}")
    ...
    companion object {
        ...
        fun newIntent(context: Context, next: Class<*>): Intent =
            Intent(context, FsiPermissionActivity::class.java)
