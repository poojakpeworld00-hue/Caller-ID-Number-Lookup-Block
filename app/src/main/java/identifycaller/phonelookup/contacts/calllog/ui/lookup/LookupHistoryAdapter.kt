...
import identifycaller.phonelookup.contacts.calllog.databinding.ItemSearchHistoryBinding
import identifycaller.phonelookup.contacts.calllog.ui.common.CallFormat

class LookupHistoryAdapter(
    private val onClick: (HistoryEntry) -> Unit,
    private val onCall: (HistoryEntry) -> Unit,
    ...
    private val onRevealName: (HistoryEntry) -> Unit = {}
) : RecyclerView.Adapter<LookupHistoryAdapter.VH>() {
    ...
    private fun isLocked(item: HistoryEntry): Boolean =
    ...
    override fun onBindViewHolder(holder: VH, position: Int) {
        ...
        with(holder.binding) {
            tvHistAvatar.text = CallFormat.initials(item.name, item.rawNumber)
