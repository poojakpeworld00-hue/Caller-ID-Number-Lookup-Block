package identifycaller.phonelookup.contacts.calllog.permission.fsi
 ...
 *  3. the global feature is enabled ([FsiConfig.enabled]),
 *  4. the user's country is allowed ([isCountryAllowed]),
 *  5. the specific surface (Screen or Dialog) is enabled.
 *
 * Nothing here is hardcoded — [FsiConfig] supplies every value from Remote Config.
 * The Screen appears once after Language; the Dialog appears in MainActivity,
 * rate-limited by `show_after_days` + `max_show_count`. Once the permission is
 * granted, neither ever shows again.
 */
object FsiPermission {
    ...
    /** Broadcast fired by [FsiWatchService] when the FSI toggle flips on. */
    const val ACTION_FSI_GRANTED =
        "identifycaller.phonelookup.contacts.calllog.action.FSI_GRANTED"
    ...
    // --- Country gate (reuses the app's existing IP + SIM country signals) ---
     ...
     * the feature is shown. Matches the IP country ([AdsPreferance.userCountry]),
     * the SIM/network ISO, and the locale country against the excluded list.
     */
    fun isCountryAllowed(context: Context, config: FsiConfig): Boolean {
        if (!config.countryFilterEnabled || config.excludedCountries.isEmpty()) {
            SafeSide.log(TAG, "country: ALLOWED (filter off or empty list)")
            return true
        }
        val excluded = config.excludedCountries
        val signals = deviceCountrySignals(context)
        if (signals.isEmpty()) {
            SafeSide.log(TAG, "country: ALLOWED (no country signal → fail-open)")
            return true
        }
        val blocked = signals.any { sig -> excluded.any { it.equals(sig, ignoreCase = true) } }
        SafeSide.log(TAG, "country: ${if (blocked) "BLOCKED" else "ALLOWED"} [signals=$signals, excluded=$excluded]")
        return !blocked
    }
    ...
    private fun deviceCountrySignals(context: Context): List<String> {
        ...
        AdsPreferance.getInstance(context).userCountry.takeIf { it.isNotBlank() }?.let { out += it.trim() }
        runCatching {
    ...
    fun canRun(context: Context, config: FsiConfig): Boolean {
    ...
    fun shouldShowScreen(context: Context, config: FsiConfig = FsiConfig.load(context)): Boolean {
        if (!canRun(context, config) || !config.screen.enabled) {
            SafeSide.log(TAG, "SCREEN → NO (canRun failed or screen.enabled=${config.screen.enabled})")
            return false
        }
        val shown = PrefsManager(context).fsiScreenShown
        val result = if (config.screen.showOnce) !shown else true
        SafeSide.log(TAG, "SCREEN → ${if (result) "YES" else "NO"} [showOnce=${config.screen.showOnce}, alreadyShown=$shown]")
        return result
    }

    /** Dialog (MainActivity): master gate + `dialog.enabled` + interval + max-count. */
    fun shouldShowDialog(context: Context, config: FsiConfig = FsiConfig.load(context)): Boolean {
        if (!canRun(context, config) || !config.dialog.enabled) {
            SafeSide.log(TAG, "DIALOG → NO (canRun failed or dialog.enabled=${config.dialog.enabled})")
            return false
        }
        val prefs = PrefsManager(context)
        val count = prefs.fsiDialogShowCount
        if (count >= config.dialog.maxShowCount) {
            SafeSide.log(TAG, "DIALOG → NO (count=$count >= max_show_count=${config.dialog.maxShowCount})")
            return false
        }
        val last = prefs.fsiDialogLastShownMs
        if (last == 0L) {
            SafeSide.log(TAG, "DIALOG → YES (first show; count=$count/${config.dialog.maxShowCount})")
            return true
        }
        val elapsedDays = (System.currentTimeMillis() - last) / (24L * 60L * 60L * 1000L)
        val result = elapsedDays >= config.dialog.showAfterDays
        SafeSide.log(TAG, "DIALOG → ${if (result) "YES" else "NO"} [elapsed=${elapsedDays}d, need=${config.dialog.showAfterDays}d, count=$count/${config.dialog.maxShowCount}]")
        return result
    }
    ...
    fun markScreenShown(context: Context) {
        PrefsManager(context).fsiScreenShown = true
        SafeSide.log(TAG, "ledger: screen marked shown")
    }
    ...
    fun markDialogShown(context: Context) {
        ...
        SafeSide.log(TAG, "ledger: dialog shown, count now ${prefs.fsiDialogShowCount}")
    }
    ...
    fun openSettings(activity: Activity, launcher: ActivityResultLauncher<Intent>) {
        ...
        if (launched) {
            SafeSide.log(TAG, "openSettings: FSI manage page launched in-task, watcher armed")
            FsiWatchService.start(activity)
        } else {
            SafeSide.error(TAG, "Failed to open FSI settings", null)
        }
    }

    /** Stop the watcher — call from the host's onResume once it's back. Idempotent. */
    fun stopWatch(context: Context) = FsiWatchService.stop(context)
}
