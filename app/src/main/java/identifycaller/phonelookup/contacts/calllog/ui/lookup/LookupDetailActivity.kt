...
import identifycaller.phonelookup.contacts.calllog.databinding.ActivityLookupDetailBinding
import identifycaller.phonelookup.contacts.calllog.databinding.ItemNicknameBinding
...
class LookupDetailActivity : BaseActivity<ActivityLookupDetailBinding>() {
    ...
    override fun initView() {
        ...
        RewardedAds.preload(this) // ready for the "Also known as" unlock

        val displayName = name?.takeIf { it.isNotBlank() } ?: getString(R.string.lookup_unknown_caller)
        binding.tvAvatar.text = CallFormat.initials(name, number)
    ...
    private fun bindStatus() {
        ...
        binding.tvStatus.apply {
            setText(textRes)
            setTextColor(color)
            backgroundTintList = ColorStateList.valueOf(ContextCompat.getColor(this@LookupDetailActivity, bg))
    ...
    private fun updateBlockState() {
        val blocked = rawNumber.isNotBlank() && BlocklistManager(this).isBlocked(rawNumber)
    ...
    private fun revealOne(nick: String) {
        ...
        if (AdsPreferance.getInstance(this).getBoolean("IsAdsON")) {
            RewardedAds().show(this) { doReveal() }
        } else doReveal()
    }
    ...
    companion object {
        ...
        fun newIntent(context: Context, r: LookupResult): Intent =
            Intent(context, LookupDetailActivity::class.java).apply {
