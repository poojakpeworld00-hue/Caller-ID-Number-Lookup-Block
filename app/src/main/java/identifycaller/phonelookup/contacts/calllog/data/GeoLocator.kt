 ...
 *
 * The country is detected **once** — ADHomeActivity resolves it early via
 * [getLocationFromIP] and stores it in [PrefsManager.homeCountryIso]. Every other
 ...
 * ip-api returns only the ISO code, so the dialing code is derived from [Countries].
 */
object GeoLocator {
     ...
     *  1. [PrefsManager.homeCountryIso] — user pick or ADHomeActivity's detection.
     *  2. [PrefsManager.geoCountryIso]  — our own previously-cached IP result.
    ...
    suspend fun detectCountry(context: Context): GeoCountry? {
        ...
        if (BuildConfig.DEBUG) {
            validCountry(DEBUG_FORCE_ISO)?.let { forced ->
                SafeSide.log(TAG, "DEBUG force country=$forced (cache/network bypassed)")
                return GeoCountry(forced, Countries.dialOf(forced).orEmpty())
            }
        }
        ...
        if (cached != null) {
            SafeSide.log(TAG, "reuse cached country=$cached (no IP call)")
            return GeoCountry(cached, Countries.dialOf(cached).orEmpty())
        }
        ...
        SafeSide.log(TAG, "cached detected country=${geo.iso} for app-wide reuse")
        return geo
    }
    ...
    private fun validCountry(raw: String?): String? {
        ...
        return iso.takeIf { Countries.dialOf(it) != null }
    }
    ...
    private suspend fun detectCountryFromIp(): GeoCountry? = withContext(Dispatchers.IO) {
        SafeSide.log(TAG, "no cache → getLocationFromIP()")
        val location = getLocationFromIP()
        if (location == null) {
            SafeSide.log(TAG, "getLocationFromIP() returned null → null")
            return@withContext null
        }
        SafeSide.log(TAG, "location: country=${location.country} code=${location.countryCode}")

        val iso = location.countryCode?.takeIf { it.length == 2 }?.uppercase()
        if (iso == null) {
            SafeSide.log(TAG, "no valid countryCode → null")
            return@withContext null
        }
        val dial = Countries.dialOf(iso).orEmpty()
        SafeSide.log(TAG, "resolved country=$iso dial=$dial")
        GeoCountry(iso, dial)
    }
}
