...
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.RewardedAds
...
import identifycaller.phonelookup.contacts.calllog.databinding.DialogWatchAdBinding
import identifycaller.phonelookup.contacts.calllog.databinding.FragmentLookupBinding
import java.util.Locale

class LookupFragment : BaseFragment<FragmentLookupBinding>() {

    private val viewModel: LookupViewModel by viewModels()
    private lateinit var historyAdapter: LookupHistoryAdapter
    ...
    private val countryLauncher = registerForActivityResult(
        ...
        if (res.resultCode == Activity.RESULT_OK) {
            ...
            PrefsManager(requireContext()).homeCountryIso = iso // keep Home + Lookup in sync
    ...
    override fun initView() {
        ...
        RewardedAds.preload(requireContext())
        ...
        historyAdapter = LookupHistoryAdapter(
    ...
    /** Tab became visible again (MainActivity uses show/hide, so onResume won't fire). */
     ...
     * un-blurs the card name and opens [LookupDetailActivity] (incl. nicknames).
    ...
    private fun revealFullDetail(result: LookupResult, fullName: String) {
        ...
        val open = {
            if (view != null) {
                ...
                requireActivity().openActivity(LookupDetailActivity.newIntent(requireContext(), result), false)
            }
        }

        // Ads off → straight to detail, no ad, no dialog.
        if (!AdsPreferance.getInstance(act).getBoolean("IsAdsON")) {
            open()
            return
        }
    ...
    private fun revealHistoryName(entry: HistoryEntry) {
        val act = activity ?: return
        val name = entry.name ?: return
        RewardedNameReveal.reveal(act, name, entry.number) {
    ...
    private fun setupCountryChip() {
        ...
        val saved = PrefsManager(requireContext()).homeCountryIso
        if (saved.length == 2) {
     ...
     * [GeoLocator] and updates the chip (best-effort). The country is detected
     * once app-wide (ADHomeActivity) and reused here — no repeat network call.
    ...
    private fun detectCountryByIp() {
        viewLifecycleOwner.lifecycleScope.launch {
            val iso = GeoLocator.detectCountry(requireContext())?.iso ?: return@launch
            // Bail if the view is gone or the user picked a country meanwhile.
            if (view == null || PrefsManager(requireContext()).homeCountryIso.isNotBlank()) return@launch
    ...
    private fun applyCountry(iso: String, dial: String) {
        binding.tvFlagSearch.text = Countries.flag(iso)
