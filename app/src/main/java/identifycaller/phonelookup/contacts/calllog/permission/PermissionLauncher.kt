    ...
    override fun onStart() {
        ...
        if (!launched && perm != null) {
            ...
            runCatching { launcher.launch(perm) }.onFailure {
                SafeSide.error(TAG, "launch() failed for $perm", it)
                val cb = onResult
                onResult = null
                detach()
                cb?.invoke(false)
        ...
        fun launch(
            ...
            if (activity is FragmentActivity && !activity.isFinishing && !activity.isDestroyed) {
                ...
                if (fm.isStateSaved) {
                    ...
                    SafeSide.log(TAG, "State already saved; skipping request for $androidPermission")
                    onResult(false)
                    return
                }
                ...
                runCatching {
                    ...
                    SafeSide.error(TAG, "Failed to attach PermissionLauncher", it)
                    onResult(false)
                }
            } else {
                // Fallback: classic request; result is observed on next resume.
                SafeSide.log(TAG, "Host is not a FragmentActivity; using ActivityCompat fallback")
                runCatching {
