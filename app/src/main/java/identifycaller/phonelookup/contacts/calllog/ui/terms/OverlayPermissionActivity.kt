 ...
 *  - shown ONCE  -> [PrefsManager.isOverlayTutorialShown]
 *  - lasts 3 sec -> [AUTO_DISMISS_MS]
 *  - the HAND    -> [startHandHint] animating R.id.ivHand
 *
 * It never navigates; [TermsActivity] owns the flow. This screen just finishes
...
class OverlayPermissionActivity : AppCompatActivity() {
    ...
    override fun onResume() {
        ...
        pollJob = lifecycleScope.launch {
            while (isActive) {
                if (OverlayUtils.isGranted(this@OverlayPermissionActivity)) {
                    finish()
                    return@launch
                }
                delay(POLL_MS)
