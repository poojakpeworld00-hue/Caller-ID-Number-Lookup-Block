...
object PermissionRepository {
     ...
     * explicitly — from the splash flow (ADHomeActivity) and from the permission
     * bottom sheet's Continue button on MainActivity. So the default targets
     * both `SplashActivity` and `MainActivity` with no delay (the trigger point
    ...
    fun reload(): List<PermissionRule> {
        val parsed = FirebasePermissionParser.parse(rawConfig())
        cached = parsed
        return parsed
    }
    ...
    fun refreshFromRemote(onReady: (() -> Unit)? = null) {
        try {
            ...
            rc.fetchAndActivate().addOnCompleteListener { task ->
                SafeSide.log(TAG, "Remote Config fetch success=${task.isSuccessful}")
                reload()
                onReady?.invoke()
            }
        } catch (e: Exception) {
            SafeSide.error(TAG, "refreshFromRemote failed; using cached config", e)
            reload()
            onReady?.invoke()
        }
    }
    ...
    private fun rawConfig(): String {
        return try {
            ...
            SafeSide.log(TAG, "No remote permission_engine config; using compiled-in default")
            DEFAULT_CONFIG
        } catch (e: Exception) {
            SafeSide.error(TAG, "Failed to read Remote Config; using compiled-in default", e)
            DEFAULT_CONFIG
        }
    }
