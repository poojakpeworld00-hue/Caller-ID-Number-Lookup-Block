...
import identifycaller.phonelookup.contacts.calllog.databinding.ActivityLookupHistoryBinding
 ...
 */
class LookupHistoryActivity : BaseActivity<ActivityLookupHistoryBinding>() {

    override val layoutId: Int = R.layout.activity_lookup_history

    private val viewModel: LookupHistoryViewModel by viewModels()
    private val adapter = LookupHistoryAdapter(
    ...
    private fun returnNumber(entry: HistoryEntry) {
    ...
    private fun revealName(entry: HistoryEntry) {
        val name = entry.name ?: return
        RewardedNameReveal.reveal(this, name, entry.number) { adapter.revealName(entry.rawNumber) }
    }
