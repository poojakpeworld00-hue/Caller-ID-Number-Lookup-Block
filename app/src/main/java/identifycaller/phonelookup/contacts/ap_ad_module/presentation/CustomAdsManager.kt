...
import identifycaller.phonelookup.contacts.ap_ad_module.data.CustomAd
...
class CustomAdsManager {
    ...
    companion object {
        private var cachedAds: List<CustomAd>? = null
    ...
    fun loadCustomAd(
        ...
        val ads: List<CustomAd> = if (cachedAds != null) {
            cachedAds!!
        } else {
            ...
            try {
                val parsed: List<CustomAd> = Gson().fromJson(json, object : TypeToken<List<CustomAd>>() {}.type)
                cachedAds = parsed
                parsed
            } catch (e: Exception) {
                fallback(container, backupImage, backupLayout, onFail)
    ...
    // THEME APPLY
    // --------------------------------------------------------
    private fun applyTheme(context: Context, view: View, pref: AdsPreferance) {
        try {
    ...
    // DATA BINDING
    // --------------------------------------------------------
    private fun bindData(view: View, ad: CustomAd) {
        // Big / Mid Native
