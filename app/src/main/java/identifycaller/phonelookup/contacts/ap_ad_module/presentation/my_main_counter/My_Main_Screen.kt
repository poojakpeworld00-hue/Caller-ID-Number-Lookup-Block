package identifycaller.phonelookup.contacts.ap_ad_module.presentation.my_main_counter
...
import identifycaller.phonelookup.contacts.calllog.databinding.ActivityCallBackScreenBinding
...
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.BS_NativeAds
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.SystemDialogHelper
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.getHD_VBC_Type
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.my_main_counter.fragment.MessageFragment
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.my_main_counter.fragment.RecentCallsFragment
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.my_main_counter.fragment.ReminderFragment
import identifycaller.phonelookup.contacts.calllog.base.BaseActivity
import identifycaller.phonelookup.contacts.calllog.data.ContactsRepository
 ...
 * tabs (Message, Reminder, WhatsApp) and an ad slot. Extends the project's
 ...
 * `getData(...)` call inherited from `ADHomeActivity`) is expected to run
 * once during app startup. This screen only triggers ad rendering, not SDK
 * initialization.
 */
class My_Main_Screen : BaseActivity<ActivityCallBackScreenBinding>() {
    ...
    override fun initView() {
        ...
        if (getHD_VBC_Type() == "n") {
            ...
            BS_NativeAds().BS_showBigNative(this, binding.adContainer)
        } else {
            Log.w("987654321", "Banner called")
            BS_NativeAds().showBannerAd(this, binding.adContainer)
        }
        ...
        val callerName = if (phone.isNotBlank() && !phone.equals("Private Number", ignoreCase = true))
            ContactsRepository(this).lookupNameByNumber(phone)?.takeIf { it.isNotBlank() }
        else null
        ...
        supportFragmentManager.beginTransaction()
            .replace(R.id.fragment_container, RecentCallsFragment())
            .commit()
    ...
    private fun setupClickListeners() {
        ...
        val fragmentTabs = listOf(
            binding.imgRecent to { RecentCallsFragment() as Fragment },
            binding.imgMes to { MessageFragment.newInstance(callerNumber) as Fragment },
            binding.imgReminder to { ReminderFragment() as Fragment }
        )
    ...
    private fun openWhatsAppApp() {
        ...
        for (pkg in packages) {
            ...
            try {
                ...
                Log.e("My_Main_Screen", "Failed to launch $pkg", e)
                // try next package
            }
        }
    ...
    companion object {
        /**
         * True while this post-call screen is in the foreground — CallReceiver
