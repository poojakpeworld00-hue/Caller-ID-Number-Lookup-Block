...
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.BannerAdObserver
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.BannerAds
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.BannerSize
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.BannerType
 ...
 *
 * In DEBUG builds every resolution is logged under the tag `ScreenAdConfig`
 * (filter logcat by that tag to verify which id/type each screen uses).
 */
object ScreenAdConfig {

    private const val TAG = "ScreenAdConfig"
    ...
    fun resolve(context: Context, screenName: String): ScreenAd {
        val pref = AdsPreferance.getInstance(context)
        ...
        // screen like MainActivity (show:false) stays hidden while still using the
    ...
    fun showAd(
        ...
        // bannerType → BannerSize + collapsible flag.
        val (size, collapsible) = when (resolved.bannerType.lowercase()) {
            "inline" -> BannerSize.INLINE to false
            "normal" -> BannerSize.NORMAL to false
            "collapsible" -> BannerSize.ADAPTIVE to true
            else -> BannerSize.ADAPTIVE to false
        }
        ...
        // disableInternalFallback=true → BannerAds reports a single onAdFailed()
        ...
        BannerAds().showBanner(
            ...
            observer = object : BannerAdObserver {
                override fun onAdFailed() {
                    if (BuildConfig.DEBUG) {
                        Log.d(TAG, "showAd($screenName) -> banner failed, fallback to native banner")
                    }
                    NativeBanner().showNativeBannerNative(activity, container, shimmer)
                }
            }
        )
    }
