package identifycaller.phonelookup.contacts.ap_ad_module.presentation.oninterAds
...
import identifycaller.phonelookup.contacts.ap_ad_module.domain.AdCounterManager.interBackCounter
...
import identifycaller.phonelookup.contacts.ap_ad_module.presentation.isNetworkConnected
...
class InterADsBack {
    ...
    fun loadBackInterAds(activity: Activity) {
        ...
        if (AdType.fromString(pref.getString("IsAdType")) == AdType.GOOGLE) {
            InterstitialAd.load(
                activity, id, req,
                object : InterstitialAdLoadCallback() {
                    ...
                    override fun onAdLoaded(ad: InterstitialAd) {
                        googleInterBack = ad
                        Log.d("InterADsBack", "Back Inter Loaded")
                        activity.safeLog("Back_Inter_Loaded")
                    }

                    override fun onAdFailedToLoad(err: LoadAdError) {
                        googleInterBack = null
                        Log.e("InterADsBack", "Back Inter Load Fail: ${err.message}")
    ...
    private fun showBackInternal(activity: Activity?, adsClose: () -> Unit) {
        ...
        val pref = AdsPreferance.getInstance(act)
        ...
        fun safeClose(reason: String) {
            ...
            Log.e("InterADsBack", "Closed: $reason")
            try {
                adsClose()
            } catch (_: Exception) {
            }
        ...
        when (AdType.fromString(pref.getString("IsAdType"))) {
            ...
            AdType.CUSTOM, AdType.UNKNOWN -> {
                if (pref.getBoolean("IsCustomADS"))
                    InterADsNormal.openDirectLink(act) { safeClose("custom_open") }
    ...
    private fun handleGoogleFail(
        ...
        if (pref.getBoolean("IsFail_FB")) {
            ...
            if (pref.getBoolean("IsCustomADS")) {
                InterADsNormal.openDirectLink(activity) { safeClose("google_fail_custom") }
    ...
    private fun showFacebookBackInter(
        ...
        if (context is Activity) FullScreenLoader.show(context, isLoader)
            ...
            fb.buildLoadAdConfig()
                .withAdListener(object : InterstitialAdListener {
                    ...
                    override fun onAdLoaded(ad: Ad?) {
                        if (context is Activity) FullScreenLoader.hide()
                        try {
                            fb.show()
                        } catch (e: Exception) {
                            onFail()
    ...
    private fun showCustomAfterFBFail(
        ...
        if (pref.getBoolean("IsCustomADS"))
            InterADsNormal.openDirectLink(context) { safeClose("fb_fail_custom") }
...
//        val url = AdsPreferance.getInstance(context).getString("DirectLink")
//
